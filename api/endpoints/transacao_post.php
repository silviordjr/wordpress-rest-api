...
function api_transacao_post($request){
    ...
    $user_id = $user->ID;
    $produto_disponivel = $request['produto']['vendido'] === '';

    if ($user_id > 0){
        $produto_slug = sanitize_text_field($request['produto']['id']);
        $produto_name = sanitize_text_field($request['produto']['nome']);
        ...
        $response = array(
            'post_author' => $user_id,
            'post_type' => 'trasacao',
            'post_title' => $comprador_id . '-' . $produto_name,
            'post_status' => 'publish',
            'meta_input' => array(
        ...
        $response = new WP_Error('autorizacao', 'Não Autorizado.', array('status' => 401));
    }
